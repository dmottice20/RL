...
from system_model import system_model, contribution_fx, approximate_bellman_equation
import matplotlib.pyplot as plt


# Approximate Value Iteration using Pre-Decision State.
# Initialize algorithm parameters and variables.
gamma, num_samples, num_reps, num_episodes, num_steps = 0.9, 2, 10, 500, 100
random.seed(1)
def alpha(m): return 0.5
...
for rep in tqdm(range(num_reps), desc='AVI Pre - {} replications'.format(num_reps)):
    ...
    for m in range(num_episodes):
        ...
        for _ in range(num_steps):
            ...
            solution = approximate_bellman_equation(state_t, v_bar, gamma, num_samples, prob, op_cost, value, cost)
            ...
            # Update our VFA...
            v_bar[state_t] = (1-alpha(m)) * v_bar[state_t] + alpha(m) * v_hat_t
